...
public class GUI extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        loadXMLButton.setText("Personen aus Helden-Export laden");
...
//TODO ADD UNDO
        ...
        savedPersonList.addMouseListener(new MouseAdapter() {
            ...
            public void check(MouseEvent e) {
                ...
                if (e.isPopupTrigger()) { //if the event shows the menu
                    savedPersonList.setSelectedIndex(savedPersonList.locationToIndex(e.getPoint())); //select the item
                    //jPopupMenu.show(jList, e.getX(), e.getY()); //and show the menu
                    JPopupMenu pMenu = new JPopupMenu();
                    //pMenu.setLocation(e.getX(), e.getY());
                    final JCheckBoxMenuItem delPItem = new JCheckBoxMenuItem("löschen");
                    pMenu.add(delPItem);
                    //pMenu.setVisible(true);
                    pMenu.show(e.getComponent(),
                            e.getX(), e.getY());

                    delPItem.addActionListener(new ActionListener() {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            int index = savedPersonList.getSelectedIndex();
                            if (index > -1) {
                                //savedPersonsCollection.persons.get(index);
                                savedPersonsCollection.removePerson(index);
                                try {
                                    Save.saveAsFile("persons.ser", savedPersonsCollection.persons);
                                } catch (IOException ex) {
                                    Logger.getLogger(GUI.class.getName()).log(Level.SEVERE, null, ex);
                ...
                erinnerungList.addMouseListener(new MouseAdapter() {
            ...
            public void check(MouseEvent e) {
                ...
                if (e.isPopupTrigger()) { //if the event shows the menu
                    erinnerungList.setSelectedIndex(savedPersonList.locationToIndex(e.getPoint())); //select the item
                    //jPopupMenu.show(jList, e.getX(), e.getY()); //and show the menu
                    JPopupMenu pMenu = new JPopupMenu();
                    //pMenu.setLocation(e.getX(), e.getY());
                    final JCheckBoxMenuItem delPItem = new JCheckBoxMenuItem("löschen");
                    ...
                    delPItem.addActionListener(new ActionListener() {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            int index = erinnerungList.getSelectedIndex();
                            if (index > -1) {
                                //savedPersonsCollection.persons.get(index);
                                ErinnerungCollection.getInstance().remove(index);
    ...
    public void openDialog() {
        ...
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            ...
            for (File file : files) {
                ...
                try {
                    Person p = HeldenParser.parseDef(fpath);
                    if (p != null) {
                        personCollection.addPerson(p);
                    }
